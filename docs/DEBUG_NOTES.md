...
## 10. 陷阱 6：墙体内瓦片接缝骗过"高度+法线"双判据（挂边误判，B1-①）

**现象**：玩家贴墙（WallGrab）沿墙下滑时，头顶每经过一个瓦片接缝就误判挂边（`WallGrab → LedgeGrab`）；Airborne 中尝试进 WallGrab 时同样被误转 `LedgeSlide`。

**原因**：垂直平台顶射线为了打到墙顶，x 偏移进墙体内部（表面内 2px）。射线在墙体内沿纵向行进时，会命中**相邻瓦片的接缝边界**——接缝处下方瓦片的上表面法线同样是 `(0,-1)`（水平朝上），且接缝高度恰好落在挂边容差内 → 高度 + 法线双判据被绕过（接缝与真实墙顶无法区分）。玩家头顶每过一个接缝（每 16px）就有约 8px 的误判窗口。

**修复**：增加**空间判据**——命中面正上方 2px 处用微型圆 `intersect_shape` 检测是否埋入碰撞体：

```gdscript
if _point_inside_world(Vector2(float(hit.position.x), float(hit.position.y) - 2.0)):
    return false  # 接缝上方仍有瓦片（墙体延续）→ 不是墙顶
```

**规律**：真实墙顶 = 墙体"结束"处（上方是空的）；接缝 = 墙体"延续"处（上方还有瓦片）。任何"从墙体内侧打向表面的射线"都必须验证表面上方是否有空间，否则内部边界（接缝）与外部边界（墙顶）无法区分。

**遗留权衡**：平台边缘的射线采样点恰好在紧邻结构正下方时（如柱子压在平台上），空间判据会拒绝挂该边缘——该处头顶净空确实受限，判定偏保守。
